...
def solve_free_conc(free_conc, total_conc, sites_conc, ori_sites_conc, active_frac, sites_per_ori, KD_s, KD_o, cascade_correct):
    if cascade_correct == 1:
        conc_ori_binding = free_conc * active_frac
    else:
        conc_ori_binding = free_conc
    return total_conc \
            - free_conc \
            - sites_conc * free_conc / (KD_s + free_conc) \
            - ori_sites_conc * (conc_ori_binding)**sites_per_ori/(KD_o**sites_per_ori + (conc_ori_binding)**sites_per_ori)
...
def plot_three_subplots_paper_style(label_list, x_axes_list, y_axes_list, color_list, t_b_list, t_init_list, v_init, file_path, file_name, title=None, h_line_plot_3 = None, period_blocked = None):
    ...
    for item in range(0, len(label_list)):
        ...
        for t_init in t_init_list:
            ax[item].axvline(x=t_init, 
                            ...
                            linestyle='--',
                            clip_on=False)
            if period_blocked is not None:
                ax[item].axvspan(t_init, 
                    t_init + period_blocked, 
                    ymin=y_min_list[item],
                    ymax=y_max_list[item],
                    alpha=0.3, 
                    color='red', 
    ...
    # ax[0].set_yticklabels(['0',r'$v^\ast_{th}$'])
    # ax[0].get_yticklabels()[1].set_color(color_list[0])
    # ax[0].axhline(y=v_init, color=color_list[0], linestyle='--')

    # ax[1].set_yticks([0,1,2])
    # ax[1].set_yticklabels([r'0',r'1', r'2'])

    if h_line_plot_3 is not None:
        ax[2].axhline(y=h_line_plot_3, color=color_list[2], linestyle='--')
        ax[2].set_yticks([0, h_line_plot_3])
        ax[2].set_yticklabels(['0', str(h_line_plot_3)])
    ...
    # ax[2].set_xticks(t_b_list)
    # ax[2].set_xticklabels(['0', '1', '2', '3'])
    plt.savefig(file_path +'/' + file_name + '.pdf', format='pdf', bbox_inches='tight')
    plt.close(fig)

def plot_four_subplots_paper_style(label_list, x_axes_list, y_axes_list, color_list, t_b_list, t_init_list, v_init, file_path, file_name, title=None, h_line_plot_3 = None, extra_second_row = None):
    fig, ax = plt.subplots(4, figsize=(6,4))
    plt.xlabel(r'time [h]')
    y_min_list = [0,0,0,0]
    y_max_list = [1, 1.2, 1.2, 1.2]
    for item in range(0, len(label_list)):
    ...
    if title is not None:
        ax[0].set_title(title)
    if extra_second_row is not None:
        ax[1].plot(x_axes_list[1], extra_second_row, color="r")
    # ax[0].set_yticks([0,v_init])
    # ax[0].set_yticklabels(['0',r'$v^\ast_{th}$'])
    # ax[0].get_yticklabels()[1].set_color(color_list[0])
    # ax[0].axhline(y=v_init, color=color_list[0], linestyle='--')
